      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.axios
            ...
            if (responseBody.state == 20000) {
              ///////////
              console.log("修改的文章的id值" + article.id);
              //将文章的id赋值给图片的对应文章id
              this.file.articleId = article.id;
              console.log("图片对应的文章Id = " + this.file.articleId)
              //图片发送异步请求
              //将封装的图片发送请求
              let pictureURI = "http://localhost:10001/articlePictures/add-new" // TODO 这里注意改一下！
